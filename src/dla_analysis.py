...
def create_separate_plots(all_data, clusters):
    ...
    plt.legend(fontsize=TICKSIZE, handlelength=0.5, handletextpad=0.3, borderpad=0.3,
    ...
    bars = plt.bar(range(len(eta_values)), growth_rates,
                   color=[colors[i % len(colors)] for i in range(len(eta_values))])
    ...
    if eta_values:
        ...
        # Custom colormap
        cmap = LinearSegmentedColormap.from_list('cluster_cmap',
                                                 [(250/255, 250/255, 250/255, 0.3), "#FFFF00"])
